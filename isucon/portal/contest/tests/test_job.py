...
from isucon.portal.contest.models import Server, Benchmarker, Job, Score
...
class JobTest(TestCase):
    ...
    def test_get_latest_score(self):
        ...
        # FIXME: seedデータに置き換え
        for i in range(10):
            want_entry = Job.objects.create(team=self.team, score=i, is_passed=True, status=Job.DONE, finished_at=timezone.now())

        # スコア履歴のもので最新のものを取得できているか
        got_entry = Job.objects.get_latest_score(self.team.id)
    ...
    def test_get_top_teams(self):
        ...
        for idx in range(10):
            ...
            # 適当にスコア獲得
            Job.objects.create(team=team, score=idx, is_passed=True, status=Job.DONE, finished_at=timezone.now())
